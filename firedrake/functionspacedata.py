...
def get_shared_data_key(mesh, element, use_true_tensor_element_shape=False):
    ...
    :arg use_true_tensor_element_shape: If True, output `nodes_per_entity`
        if expanded to represent the true `ufl.TensorElement` shape.
    """
    ...
    if use_true_tensor_element_shape:
        if type(element) is ufl.TensorElement:
            shape = element.reference_value_shape()
        elif type(element) is ufl.VectorElement:
            shape = element.value_shape()[:1]
        else:
            raise ValueError("`use_true_tensor_element_shape` can not be ``True`` if `element` is not an instance of `ufl.TensorElement`.")
        size = numpy.product(shape)
        nodes_per_entity = tuple(size * n for n in nodes_per_entity)
