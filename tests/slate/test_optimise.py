...
def test_push_assembled_vector_blocks_individual(TC):
    ...
    T, C = TC
    expressions = [(C+C).blocks[0], (T*C).blocks[0], (-C).blocks[0], (C.T).blocks[0]]
    compare_vector_expressions(expressions)
...
def compare_vector_expressions(expressions):
    for expr in expressions:
        ref = assemble(expr, form_compiler_parameters={"optimise_slate": False}).dat.data
        opt = assemble(expr, form_compiler_parameters={"optimise_slate": True}).dat.data
        assert np.allclose(opt, ref, rtol=1e-14)
